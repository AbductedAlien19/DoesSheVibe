...
export function AppProvider({ children }: { children: ReactNode }) {
      ...
      .on(
        ...
        (payload) => {
          ...
          setChatMessages((prev) => {
            ...
            const withoutTemp = prev.filter((m) => !m.id.startsWith('temp-') || m.message !== newMsg.message || m.username !== newMsg.username);
  ...
  const sendMessage = useCallback(async (msg: string, username: string) => {
    ...
    try {
      ...
      // message already shown optimistically; realtime will sync if available
    }
  }, []);
